...
for severity in range(3):
    ...
    for prompt_idx, ref_text in enumerate(ref_texts):
        ...
        word_path = os.path.join(OUT_DIR, f"word_dys_{label}_p{prompt_idx}.txt")
...
"""
        ...
        ipa_path = os.path.join(OUT_DIR, f"phonemes_{label}_p{prompt_idx}.txt")
