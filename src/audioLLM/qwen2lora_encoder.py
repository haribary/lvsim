...
# Load model + LoRA on audio encoder only (decoder frozen)
...
# Freeze decoder (language model)
for param in model.language_model.parameters():
    param.requires_grad = False

# Unfreeze multi-modal projector (bridges encoder -> decoder)
for param in model.multi_modal_projector.parameters():
    param.requires_grad = True

# LoRA on audio encoder attention only (q_proj, v_proj in audio_tower)
lora_config = LoraConfig(
    r=16,
    lora_alpha=32,
    lora_dropout=0.05,
    target_modules=r"audio_tower\.layers\.\d+\.self_attn\.[qv]_proj",
    bias="none",
)
...
print("Model ready (encoder LoRA + projector fine-tune, decoder frozen).")
...
ax.set_xlabel("Epoch")
ax.set_ylabel("Loss")
ax.set_title("Qwen2-Audio Encoder LoRA Training")
ax.legend()
fig.tight_layout()
fig.savefig(os.path.join(MISC_DIR, "qwen2_encoder_lora_training_curves.png"), dpi=150)
plt.close(fig)
print(f"Saved to {MISC_DIR}/qwen2_encoder_lora_training_curves.png")

# Save LoRA adapters
save_dir = os.path.join(BASE_DIR, "data", "models", "qwen2_audio_encoder_lora")
...
print("=" * 60)
print("Overall Classification Report (Encoder LoRA Fine-Tuned Qwen2-Audio)")
print("=" * 60)
